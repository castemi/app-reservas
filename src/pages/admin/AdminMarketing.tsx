  ...
  const handleSend = async () => {
    ...
    try {
      if (tipo === 'inmediata') {
        ...
        // Rate limiting: el servidor devuelve 200 con { error: 'rate_limited', ... }
        if (fnData?.error === 'rate_limited') {
          toast({
            title: '⏱️ Espera un momento',
            description: fnData.message || `Debes esperar ${fnData.remaining ?? 60} segundos antes de enviar otra campaña.`,
            variant: 'destructive',
          });
          setSending(false);
          return;
        ...
        if (fnData?.sent === 0) throw new Error('No se pudo enviar ningún email. Revisa los logs.');

        toast({ title: `✅ Email enviado a ${fnData?.sent ?? 1} destinatario(s)` });
