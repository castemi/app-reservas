...
export default function Empresa() {
  ...
  const { data: settings, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('business_settings').select('id, nombre, email_contacto, telefono, direccion, horario_apertura, horario_cierre, dias_laborables').limit(1).single();
